...
(function (global) {
  ...
  const SectionCard = React.forwardRef(({ id, title, description, children }, ref) => (
    <section
      ref={ref}
      id={id}
      className="scroll-mt-32 rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/70 backdrop-blur"
    >
      ...
      <div className="glass-card space-y-4 p-5">
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-800">Review & Tune</p>
          </div>
          <span className="rounded-full bg-blue-50 px-3 py-1 text-[11px] font-semibold text-blue-700">Guided</span>
        </div>
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm font-medium transition ${
                  isActive
                    ? 'bg-blue-50 text-blue-800 shadow-inner shadow-blue-100'
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <span className="block text-[11px] text-slate-400">Tap to jump</span>
                </div>
                {isActive ? <span className="text-sm">➜</span> : null}
              </button>
            );
          })}
        </nav>
        {isDirty && (
          <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            Unsaved changes detected. Save to apply.
          </p>
        )}
      </div>
    </aside>
    ...
    return (
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {chips.map((chip, index) => (
          <div
            key={index}
            className="flex flex-col gap-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 shadow-inner"
          >
            <span className="text-[11px] uppercase tracking-wide text-slate-500">{chip.label}</span>
            <span className="text-base font-semibold text-slate-900">{chip.value}</span>
          </div>
        ))}
      </div>
    );
    ...
    <div className="mb-8">
      <div className="glass-card flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-800">Navigate configuration, keys, and patterns</p>
        </div>
