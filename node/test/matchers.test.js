...
import { _evaluateMatchers, _selectApiKeyForPattern } from '../src/pipeline/proxyPipeline.js';

describe('evaluateMatchers', () => {
  it('returns matched when no matchers provided', () => {
    expect(_evaluateMatchers({})).toEqual({ matched: true });
  });

  it('rejects when JSON missing and matchers require data', () => {
    const res = _evaluateMatchers(undefined, [{ path: '.foo', exists: true }]);
  ...
  it('validates exists/equals/contains semantics', () => {
    ...
    expect(_evaluateMatchers(parsed, [
      { path: '.message.content', contains: 'world' },
      { path: '.message.safe', equals: true },
      { path: '.message.content', exists: true },
    ])).toEqual({ matched: true });

    const mismatch = _evaluateMatchers(parsed, [{ path: '.message.content', equals: 'nope' }]);
  ...
  it('handles array index shorthand and missing accessors', () => {
    ...
    const ok = _evaluateMatchers(parsed, [{ path: '.messages[-1].content', equals: 'b' }]);
    expect(ok.matched).toBe(true);

    const bad = _evaluateMatchers(parsed, [{ path: '.messages[5].content', exists: true }]);
...
describe('selectApiKeyForPattern', () => {
  ...
  it('skips running when matchers miss', () => {
    ...
    const res = _selectApiKeyForPattern(context, pattern, [{ name: 'k1', key: 't' }], 'fallback', logger, 'request');
  ...
  it('returns api key token when pattern matches', () => {
    ...
    const res = _selectApiKeyForPattern(context, pattern, apiKeys, 'fallback', logger, 'response');
  ...
  it('falls back to default bearer when api key missing', () => {
    ...
    const res = _selectApiKeyForPattern(context, pattern, [], 'fallback-token', logger, 'response');
