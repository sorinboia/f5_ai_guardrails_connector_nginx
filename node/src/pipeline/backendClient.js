...
function createStreamDestroyer(reply, source, tee) {
  return (err) => {
...
function setupStreamErrorHandlers(source, tee, reply, logger) {
  source.on('error', (err) => {
...
function logBlockedChunk(logger, result) {
  logger?.warn?.({
    step: 'stream:passthrough_drop',
    reason: 'live_chunk_blocked',
    api_key_name: result.apiKeyName,
    pattern_id: result.patternId,
    details: result.details || {}
  });
}

async function runGatedStream(source, tee, inspectChunk, destroyStreams, logger) {
  let responseBody = '';
  let blocked = false;
  ...
  return { responseBody, blocked };
}

function runPassthroughStream(source, tee, inspectChunk, destroyStreams, logger) {
  let responseBody = '';
  let blocked = false;
  let inspecting = false;
...
export async function fetchStream(url, init, reply, inspectChunk, options = {}, logger) {
  ...
  const destroyStreams = createStreamDestroyer(reply, source, tee);

  setupStreamErrorHandlers(source, tee, reply, logger);

  let responseBody = '';

  if (gateChunks) {
    reply.send(tee);
    const result = await runGatedStream(source, tee, inspectChunk, destroyStreams, logger);
    ...
    const passthrough = runPassthroughStream(source, tee, inspectChunk, destroyStreams, logger);
    reply.send(tee);
    ...
    responseBody = passthrough.getResponseBody();
  }
