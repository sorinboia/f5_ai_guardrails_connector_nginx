...
- **Auxiliary assets**: mitmdump CA downloads at `/config/mitm/mitmproxy-ca-cert.pem` (PEM) and `/config/mitm/mitmproxy-ca-cert.cer` (DER). Files are served from `/var/lib/mitmproxy/` and return 404 until mitmdump has generated them.
...
- `Dockerfile` — image build copying configs, njs scripts, certs, mitmdump addon, exposing 11434/11443/10000, and exporting mitmdump CA downloads from `/var/lib/mitmproxy/`.
