...
- ✅ Phases 1–6 completed (parity mapping, architecture spike, core pipeline, management APIs, static/MITM, unit coverage).
- ✅ Phase 7 completed: Dockerfile now builds a Node-only image and runs `mitmdump` + Node server. NGINX artifacts removed from the repo and runtime.
...
2) **Performance & resiliency**: Capture latency/throughput versus the former NGINX stack; validate fail-open/closed toggles and TLS reload paths.
...
## Cutover Notes
- Ports remain the same (11434/11443); MITM sidecar still listens on 10000.
- Rollback requires checking out a git revision prior to this cutover to restore NGINX configs/scripts.
- Static assets, cert locations, and MITM download paths remain unchanged so the UI and CA distribution continue to work.

## Checklist
- [x] Node parity features implemented (request/response/stream inspection, redaction, collector, management APIs, static UI, MITM CA downloads).
