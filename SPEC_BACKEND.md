# F5 AI Guardrails Connector — Backend Specification

Scope: Node/Fastify service that inspects HTTP(S) traffic, applies Guardrails decisions, exposes management APIs, and hosts static UI assets. This file is the authoritative contract for backend behaviour, ports, headers, pipeline stages, persistence, and operational commands.

---
## 1) Runtime Topology
- **Entrypoint**: `node/src/server.js` boots two Fastify instances — management/UI on HTTP `22100`, data plane proxy on HTTP `22080`, and HTTPS `22443` when cert/key are present. TLS assets default to `../certs/sideband-local.crt|key`. When `DYNAMIC_CERTS_ENABLED=true` and `MITM_CA_CERT`/`MITM_CA_KEY` are readable, the HTTPS listener issues per-host leaf certificates signed by the provided CA; otherwise the static cert is served.
- **Static assets**: React SPA bundle (from `ui/dist`) resides in `html/` and is served by `node/src/routes/static.js` (`index.html` + hashed assets). MITM/CA download endpoints expose the active root at `/config/mitm/ca.pem` and `/config/mitm/ca.cer`; legacy mitmdump paths remain for compatibility.
- **Config & state**: Persisted JSON file at `var/guardrails_config.json` (override via `CONFIG_STORE_PATH`). `server.js` watches the file for hot reload and mutates the in-memory store in place so route decorators remain live.
- **Forward proxy**: `node/src/forwardProxy.js` runs an HTTP/1.1 listener on `0.0.0.0:10000` handling absolute-form requests and `CONNECT` tunnels. Destinations must already exist as hosts in the config store; unlisted hosts are rejected with `403`. Allowed targets are relayed to the local data-plane listeners (`http 22080` or `https 22443`) so the same inspection pipeline runs. HTTPS CONNECTs inherit the data-plane TLS behaviour (static or dynamic certs).
- **Logging**: Pino logger (`node/src/logging/logger.js`); base level from env `LOG_LEVEL` (default `info`). Effective level is resolved per request from host config `logLevel` (inherits from `__default__`) and can be overridden via `X-Sideband-Log` (`debug|info|warn|err`). Logs emit lower_snake_case fields.
...
- **Static/UI**: `/config/ui`, `/config/ui/keys`, `/config/ui/patterns`, `/config/ui/*` all serve the React SPA `index.html` (no-store cache headers). `/collector/ui` redirects to `/config/ui`. Served only from the management listener.
